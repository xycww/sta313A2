...
# Restrict to ages 25–44 and build a child-status flag
df_25_44 <- clean_data %>%
  mutate(
    Gender = recode(as.character(Gender), `1`="Women", `2`="Men"),
    ChildStatus = case_when(
...
# Median wages by ChildStatus x Gender, then compute gap = (Men - Women) / Men
gap_child <- df_25_44 %>%
  group_by(ChildStatus, Gender) %>%
  summarise(med = median(Wages, na.rm = TRUE), .groups = "drop") %>%
  tidyr::pivot_wider(names_from = Gender, values_from = med) %>%
  # keep only statuses where we have both genders
  filter(!is.na(Men), !is.na(Women)) %>%
  mutate(
    gap = (Men - Women) / Men,
    ChildStatus = factor(ChildStatus, levels = c("No children","Has children"))
  ) %>%
  arrange(ChildStatus)

# Plot: two bars = with kids vs without kids
ggplot(gap_child, aes(x = ChildStatus, y = gap, fill = ChildStatus)) +
  ...
  scale_y_continuous(labels = label_percent(accuracy = 1),
                     expand = expansion(mult = c(0.02, 0.08))) +
  labs(
    title = "Gender pay gap (ages 25–44): with vs without children",
    subtitle = "Gap = (Men median − Women median) / Men median, using medians within each group",
    x = NULL, y = "Gap percentage"
  ) +
  theme_minimal(base_size = 12) +
  theme(panel.grid.minor = element_blank())
